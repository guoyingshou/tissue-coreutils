...
public class PlanDaoImpl implements PlanDao {
    ...
    public void addMember(String planId, String accountId) {
        OrientGraph db = dataSource.getDB();
        try {
            String sql = "create edge Members from " + accountId + " to " + planId + " set createTime = sysdate(), category ='member'";
            logger.debug(sql);
    ...
    public Boolean isMember(String planId, String accountId) {
        String sql = "select from " + planId + " where " + accountId + " in set(in_Owns.out, in_Members.out)";
        logger.debug(sql);
    ...
    public List<Plan> getPlansByUser(String userId) {
        String sql = "select @this as plan, in_Contains as topic " + 
                     "from Plan " +
                     "where set(in_Owns.out.out_Belongs, in_Members.out.out_Belongs) in " + userId;
        logger.debug(sql);
    ...
    public List<Plan> getPlansByAccount(String accountId) {
        String sql = "select @this as plan, " +
                     "in_Contains as topic " + 
                     "from plan " +
                     "where set(in_Owns.out, in_Members.out) in " + accountId;
        logger.debug(sql);
